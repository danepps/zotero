...
Straight apostrophes are converted to their typographic forms before injection. citeproc runs every string it renders through its flip-flop parser, which turns a straight `'` (U+0027 — what users type into Zotero) into U+2019 for possessives, contractions, and name particles, and into a U+2018/U+2019 pair around a quoted phrase; injected metadata has to match or the same source reads two different ways in one document. `BCF.cite.smartApostrophes(s)` handles titles and other running text — it's folded into `normalizeTitleMarkup`, so `shortTitle` / `fullTitle` / `titleSegments` are already normalized — and `BCF.cite.smartNameApostrophes(s)` handles personal names, where a leading apostrophe is a particle (`'t Hooft`), not an opening quote. `BCF.cite.surnames()` applies the name variant, so `authorKey` also buckets the two spellings of `O'Connor` together. Both helpers are idempotent (they only touch U+0027).
...
- Smart-apostrophe conversion matches citeproc except at the head of a string: a title *opening* with an apostrophe that isn't a quote (`'Tis the Season`) becomes U+2018 here, where citeproc emits U+2019.
