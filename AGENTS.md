...
**Both paths run, always — the redundancy is load-bearing.** Features are idempotent, so the `setText` hook re-running the chain on text the prewrite pass already rewrote is harmless, and it covers any field write whose text didn't flow through `citation.text`. (A v1.2.0 attempt to short-circuit `setText` while `_updateDocument` was active broke rewrites in real documents and was reverted — don't reintroduce it without testing in a live Word session.) Both hook paths are gated on **RTF output** via `BCF.patch._sessionOutputFormat`, which reads **only `session.outputFormat`** (the exact field the setText gate always used) and fails open when unset: HTML (Google Docs) and plain-text sessions pass through untouched, since the chain emits RTF fragments.
...
- `[hereinafter ...]` is always appended at the **end** of the segment, after any suffix. Rule 4.2(b) technically wants it before an *explanatory* parenthetical, but a cite's suffix can just as easily hold the date/edition parenthetical (which the bracket must follow), and the two can't be told apart reliably. A v1.2.0 attempt to insert before every suffix misplaced brackets in real documents and was reverted — don't reintroduce without an explicit user signal distinguishing the suffix kinds.
